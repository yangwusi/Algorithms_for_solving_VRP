# -*- coding: utf-8 -*-
# @Time    : 2021/12/13 10:40
# @Author  : Praise
# @File    : SA_MDHFVRPTW.py
# obj:
        ...
        for j in range(i + 1, len(model.demand_id_list)):
            ...
            dist = math.sqrt(
                (model.demand_dict[from_node_id].x_coord -
                 model.demand_dict[to_node_id].x_coord) ** 2 +
                (
                    model.demand_dict[from_node_id].y_coord -
                    model.demand_dict[to_node_id].y_coord) ** 2)
            model.distance_matrix[from_node_id, to_node_id] = dist
            model.distance_matrix[to_node_id, from_node_id] = dist
        for _, vehicle in model.vehicle_dict.items():
            dist = math.sqrt(
                (model.demand_dict[from_node_id].x_coord - vehicle.x_coord) ** 2 + (
...
def checkTimeWindow(route, model, vehicle):
    ...
    for i in range(len(route)):
        if i == 0:
            ...
            travel_time = int(
                model.distance_matrix[vehicle.type, next_node_id] / vehicle.free_speed)
            departure = max(
                0,
                model.demand_dict[next_node_id].start_time -
                travel_time)
            ...
            travel_time = int(
                model.distance_matrix[last_node_id, current_node_id] / vehicle.free_speed)
            arrival = max(timetable[-1][1] + travel_time,
                          current_node.start_time)
            ...
            last_node_id = route[i - 1]
            travel_time = int(
                model.distance_matrix[last_node_id, vehicle.type] / vehicle.free_speed)
...
def checkResidualCapacity(residual_node_id_list, W, model):
    ...
    for k, v_type in enumerate(model.vehicle_type_list):
        ...
        residual_fleet_capacity += (vehicle.numbers -
                                    W[k + 4]) * vehicle.capacity
    ...
    for i in range(model.number_of_demands):
        ...
        while True:
            ...
            for k, v_type in enumerate(model.vehicle_type_list):
                vehicle = model.vehicle_dict[v_type]
                if i == j:
                    distance[v_type] = model.distance_matrix[v_type,
                                                             n_1] + model.distance_matrix[n_1, v_type]
                else:
                    n_3 = node_id_list[j - 1]
                    distance[v_type] = distance[v_type] - model.distance_matrix[n_3, v_type] + \
                        model.distance_matrix[n_3, n_2] + model.distance_matrix[n_2, v_type]
                route = node_id_list[i:j + 1]
                route.insert(0, v_type)
                route.append(v_type)
                if not checkTimeWindow(
                        route, model, vehicle):  # 检查时间窗，只有满足时间窗才有可能生成新的标签，否则跳过
                    continue
                for id, label in enumerate(V[i - 1]):
                    if load <= vehicle.capacity and label[k +
                                                          4] < vehicle.numbers:
                        stop = True
                        "计算路径成本，这里计算旅行时间成本时，只考虑节点间的旅行时间，暂不考虑等待时间成本"
                        if model.opt_type == 0:
                            cost = vehicle.fixed_cost + \
                                distance[v_type] * vehicle.variable_cost
                        else:
                            cost = vehicle.fixed_cost + \
                                distance[v_type] / vehicle.free_speed * vehicle.variable_cost
                        ...
                        if checkResidualCapacity(
                                node_id_list[j + 1:], W, model):
                            "根据帕累托将W插入到当前节点的标签列表中，同时删除被支配标签"
                            label_list, number_of_lables = updateNodeLabels(
                                V[j], W, number_of_lables)
                            V[j] = label_list
            j += 1
...
def calTravelCost(route_list, model):
    ...
    for route in route_list:
        ...
        for i in range(len(route)):
            if i == 0:
                next_node_id = route[i + 1]
                travel_time_between_nodes = model.distance_matrix[v_type,
                ...
                current_node_id = route[i]
                current_node = model.demand_dict[current_node_id]
                travel_time_between_nodes = model.distance_matrix[last_node_id,
                                                                  current_node_id] / free_speed
                arrival = max(
                    timetable[-1][1] + travel_time_between_nodes, current_node.start_time)
                departure = arrival + current_node.service_time
                timetable.append((int(arrival), int(departure)))
                travel_distance += model.distance_matrix[last_node_id,
                                                         current_node_id]
                ...
                travel_time += model.distance_matrix[last_node_id,
                                                     v_type] / free_speed
...
def calObj(sol, model):
    ...
    if ret is not None:
        sol.route_list = ret
        sol.timetable_list, sol.time_of_routes, sol.distance_of_routes, sol.obj = calTravelCost(
...
def doAction(node_id_list, action):
    ...
    if action[0] == 1:
        index_1 = action[1]
        index_2 = action[2]
        node_id_list_[index_1], node_id_list_[
            index_2] = node_id_list_[index_2], node_id_list_[index_1]
...
def run(demand_file, depot_file, T0, Tf, deltaT, opt_type):
    ...
    while Tk >= Tf:
        ...
        print("当前温度：%s，local obj:%s best obj: %s, runtime: %s" %
              (Tk, sol.obj, model.best_sol.obj, time.time() - start_time))
...
if __name__ == '__main__':
    ...
    run(demand_file=demand_file,depot_file=depot_file,T0=6000,Tf=0.001,deltaT=0.9,opt_type=0)
